...
import integration.core.dto.mapper.InboxEventMapper;
...
import integration.core.runtime.messaging.repository.InboxEventRepository;
...
public class InboxServiceImpl implements InboxService {
    ...
    @Autowired
    private InboxEventRepository eventRepository;
    ...
    @Override
    public void recordEvent(long messageFlowId, long componentId, long routeId, String owner, String jmsMessageId) throws OutboxEventProcessingException, MessageFlowNotFoundException, ComponentNotFoundException {
        try {
            Optional<InboxEvent>inboxEvent = eventRepository.findExistingEvent(componentId, jmsMessageId);
             ...
             event.setJmsMessageId(jmsMessageId);
    ...
    public void recordEvent(long messageFlowId, long componentId, long routeId, String owner) throws OutboxEventProcessingException, MessageFlowNotFoundException, ComponentNotFoundException {
        try {      
            ...
            IntegrationComponent integrationComponent = messageFlow.getComponent();
            if (integrationComponent == null) {
                throw new ComponentNotFoundException(componentId);
            }

            event.setComponent(integrationComponent);
            event.setRoute(integrationComponent.getRoute());
            event.setOwner(owner);
    ...
    @Override
    public List<InboxEventDto> getEventsForComponent(long componentId, int numberToRead,List<Long>processedEventIds) throws OutboxEventProcessingException {
        try  {
            InboxEventMapper mapper = new InboxEventMapper();
            List<InboxEventDto> eventDtos = new ArrayList<>();
               
            List<InboxEvent> events = null;
    ...
    public void markEventForRetry(long eventId, IntegrationException theException) throws MessageFlowProcessingException, OutboxEventNotFoundException, OutboxEventProcessingException {
        try {
            Optional<InboxEvent> eventOptional =  eventRepository.findById(eventId);
            if (eventOptional.isEmpty()) {
                throw new OutboxEventNotFoundException(eventId);
            }
            
            InboxEvent event = eventOptional.get();
